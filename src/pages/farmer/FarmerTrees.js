...
import { FaTree, FaSearch, FaFilter, FaEye, FaExclamationTriangle } from 'react-icons/fa';
...
const FarmerTrees = () => {
  ...
  const getHealthBadge = (health) => {
    ...
    return (
      <span className={`${colors[health] || 'bg-gray-100 text-gray-800'} text-xs px-2 py-1 rounded-full`}>
        {health.charAt(0).toUpperCase() + health.slice(1)}
      </span>
    );
  };

  const handleReportDeath = (treeId) => {
    toast.success(`Report death initiated for ${treeId} (mock)`);
  };
    ...
    <div className="min-h-screen bg-gray-100">
      <div className="container mx-auto px-4 py-8">
        ...
        >
          ...
          <p className="text-gray-600 mt-2">View and manage trees on your land.</p>
        </motion.div>
              ...
              <tbody className="divide-y divide-gray-200">
                {filteredTrees.map((tree) => (
                  <tr key={tree.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4">{getHealthBadge(tree.health)}</td>
                    <td className="px-6 py-4">
                      <div className="flex space-x-2">
                        <Link
                          to={`/farmer/trees/${tree.id}`}
                        ...
                        {tree.status === 'dead' && (
                          <button
                            onClick={() => handleReportDeath(tree.id)}
